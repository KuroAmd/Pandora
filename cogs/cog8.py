...
class Fun(commands.Cog):
    ...
    # CERN
    @commands.command(aliases=['CERN',"المنظمة","سیرن"])
...
#just embed it later
    ...
    @commands.command(aliases=["انتهی","بوري"])
    async def wasted(self, ctx, u: discord.Member=None):
        if not u:
            u = ctx.author
        
        wastedsession = aiohttp.ClientSession()
        async with wastedsession.get(f"https://some-random-api.ml/canvas/wasted?avatar={u.avatar_url_as(format='png')}") as img:
            if img.status != 200:
    ...
    @commands.command(aliases=['animalfact','afact','حیوانات'],description="about different animals (but specify which ^_^)")
    ...
    @commands.command(aliases=['8ball','question','q',"سوال"],description="Ask and I shall answer!~")
    async def eightball(self,ctx,* ,question):
      responses= ["Yes!", "Of course!", "Why not?", "Sure!", "Certainly",
      "100 Percent! \n~~unless my calculations are wrong... which is likely~~", "I'd say... Definitely!", "There's no doubt about it!", "Possible...", "Maybe~", "Don't lose hope~!", "Uhh... I'm not sure", "I DON'T KNOW! >_<", "I wonder... maybe?",
      "Very doubtful","I don't think so", "And my answer will be... no", "Of course not!", "No way", "Nah", "Nuh-uh!", "And no", "N O",
      "ahhh My head hurts x_x", "Need to think about that... Ask again"]
      await ctx.send(f"You ask: {question}?")
      await ctx.send(f"{random.choice(responses)} {db['mood']}")
...
# # Animec api
    ...
    async def slap(self,ctx,u:discord.Member=None):
      if not u:
        u= ctx.author
      a= animec.Waifu.slap()
      em= Embed(title=f"slap! Ouch",colour=u.colour)
      em.set_image(url=a)
      em.set_footer(text= "-", icon_url= ctx.author.avatar_url)
      await ctx.send(embed=em)
    ...
    @commands.command(aliases=['eat',"عم",""])
    ...
    async def glomp(self,ctx,u:discord.Member=None):
      ...
      em= Embed(title=f"aw don't be sad, {u.name}!",colour=u.colour)
      em.set_image(url=a)
      em.set_footer(text= "-", icon_url= ctx.author.avatar_url)
      await ctx.send(embed=em)
